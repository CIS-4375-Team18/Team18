        ...
        <q-card style="width: 300px" class="adddialogcard">
          ...
          <q-card-section>
            <q-input
              clearable
              color="secondary"
              v-model="newCatDescrData.HARDW_SUB_CATEGORY_DESC"
...
export default {
  ...
  data() {
    return {
      ...
      hardwCategoryData: [], // Holds the category data retrieved from the database
      updatedhardwCategoryData: [],
      tableKey: 0, // Used to re-render q-tables after a row has been deleted
      catEmptyInput: false,
    };
  },
  created() {
    this.getHardwCategories(); // Method which calls the GET API for Categories
  ...
  methods: {
    ...
    editRow(type, props) {
      ...
      itemId = props.row.HARDW_SUB_CATEGORY_ID;
      itemDesc = props.row.HARDW_SUB_CATEGORY_DESC;
      ...
      this.editedItem.itemType = "HARDW_SUB_CATEGORY";
         // show dialog
    ...
    updateCategoryAsync() {
      return new Promise((resolve, reject) => {
        axios
          .put(
            `http://localhost:8001/api/hardwcategory/${this.updateCatData.HARDW_SUB_CATEGORY_ID}`,
            this.updateCatData
          )
          .then(() => {
            this.tableKey += 1;
            this.getHardwCategories();
            resolve();
          });
      });
    },
    ...
    async handleSaveDialog(response) {
      ...
      const { item } = response;
      this.updateCatData.HARDW_SUB_CATEGORY_ID = item.HARDW_SUB_CATEGORY_ID;
      this.updateCatData.HARDW_SUB_CATEGORY_DESC = item.HARDW_SUB_CATEGORY_DESC;
    ...
    closeCategoryDialog() {
      this.newCatDescrData.HARDW_SUB_CATEGORY_DESC = ""; // Clear new category description
    ...
    deleteCategoryAsync() {
      return new Promise((resolve, reject) => {
        axios
          .delete(
            `http://localhost:8001/api/hardwcategory/${this.deleteItem.itemId}`
          )
          .then(() => {
    ...
    hideColums() {
      if (this.userRole != "IT Teacher") {
        this.visibleCategoryColumns = ["soft_category_type",
        "hardw_category_type",
        "status",
        "actions"];
      }
    ...
    async addNewCategory() {
      if (this.newCatDescrData.HARDW_SUB_CATEGORY_DESC == "") {
      ...
      this.getHardwCategories(); // Call GET API for categories
      this.newCatDescrData.HARDW_SUB_CATEGORY_DESC = ""; // Clear new category description
    },
    ...
    addNewCategoryPromise() {
      return new Promise((resolve, reject) => {
        axios
          .post(`http://localhost:8001/api/hardwcategory`, this.newCatDescrData)
          .then(() => {
            this.addCatCard = false;
            this.tableKey += 1;
            this.getHardwCategories();
    ...
    getHardwCategories() {
      axios.get(`http://localhost:8001/api/hardwcategoriesjoin`).then((res) => {
        this.hardwCategoryData = res.data;
        this.loading = false;
      });
    },
  ...
  setup() {
    const catColumns = [
      {
        name: "hardw_category_type",
        label: "Hardware Category",
        field: "HARDW_SUB_CATEGORY_DESC",
        align: "left",
      },
      {
        name: "status",
