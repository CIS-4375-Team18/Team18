        ...
        <q-card class="requests-card q-ma-lg">
          ...
          <div class="q-pa-lg">
            ...
            <q-table
              ...
              title="Subcategory Types"
              :key="tableKey"
              color="secondary"
              :align="left"
              :loading="loading"
              :rows="softwCategoryData"
              :columns="catColumns"
              virtual-scroll
              ...
              <template #body-cell-status="props">
                ...
                <q-td :props="props">
                  <q-btn
                    ...
                    color="secondary"
                    size="12px"
                    @click="editRow('Software Category', props)"
              ...
              <template v-slot:body-cell-delactions="props">
                <q-td :props="props">
                  <q-btn
                    ...
                    round
                    flat
                    @click="deleteRow('Software Category', props)"
      ...
      <q-dialog v-model="addCatCard">
        <q-card style="width: 300px" class="adddialogcard">
          <q-card-section class="qc-header text-white">
            <div class="header-text">Add New Software Category:</div>
          </q-card-section>
          <q-card-section>
            <q-input
              clearable
              color="secondary"
              v-model="newCatDescrData.SOFTW_SUB_CATEGORY_DESC"
              label="Enter New Software Category Description:"
              stack-label
              :dense="dense"
              autofocus
            />
...
export default {
  ...
  data() {
    return {
      newCatDescrData: {
        ...
        SOFTW_SUB_CATEGORY_DESC: "",
        ACTIVE_STATUS_ID: "1",
      },
      // Holds the updated category data
      updateCatData: {
        SOFTW_SUB_CATEGORY_ID: "",
        SOFTW_SUB_CATEGORY_DESC: "",
        ACTIVE_STATUS_ID: "",
      },
      newTicketCat: [],
      ...
      softwCategoryData: [], // Holds the category data retrieved from the database
      updatedsoftwCategoryData: [],
      tableKey: 0, // Used to re-render q-tables after a row has been deleted
      catEmptyInput: false,
    };
  },
  created() {
    this.getSoftwCategories(); // Method which calls the GET API for Categories
  ...
  methods: {
    ...
    editRow(type, props) {
      ...
      itemId = props.row.SOFTW_SUB_CATEGORY_ID;
      itemDesc = props.row.SOFTW_SUB_CATEGORY_DESC;
      ...
      this.editedItem.itemType = "SOFTW_SUB_CATEGORY";
         // show dialog
    ...
    updateCategoryAsync() {
      return new Promise((resolve, reject) => {
        axios
          .put(
            `http://localhost:8001/api/softwcategory/${this.updateCatData.SOFTW_SUB_CATEGORY_ID}`,
            this.updateCatData
          )
          .then(() => {
            this.tableKey += 1;
            this.getSoftwCategories();
            resolve();
          });
      });
    },
    ...
    async handleSaveDialog(response) {
      ...
      const { item } = response;
      this.updateCatData.SOFTW_SUB_CATEGORY_ID = item.SOFTW_SUB_CATEGORY_ID;
      this.updateCatData.SOFTW_SUB_CATEGORY_DESC = item.SOFTW_SUB_CATEGORY_DESC;
    ...
    closeCategoryDialog() {
      this.newCatDescrData.SOFTW_SUB_CATEGORY_DESC = ""; // Clear new category description
    ...
    deleteCategoryAsync() {
      return new Promise((resolve, reject) => {
        axios
          .delete(
            `http://localhost:8001/api/softwcategory/${this.deleteItem.itemId}`
          )
          .then(() => {
    ...
    hideColums() {
      if (this.userRole != "IT Teacher") {
        this.visibleCategoryColumns = ["softw_category_type",
        "status",
        "actions"];
    ...
    async addNewCategory() {
      if (this.newCatDescrData.SOFTW_SUB_CATEGORY_DESC == "") {
      ...
      this.getSoftwCategories(); // Call GET API for categories
      this.newCatDescrData.SOFTW_SUB_CATEGORY_DESC = ""; // Clear new category description
    },
    ...
    addNewCategoryPromise() {
      return new Promise((resolve, reject) => {
        axios
          .post(`http://localhost:8001/api/softwcategory`, this.newCatDescrData)
          .then(() => {
            this.addCatCard = false;
            this.tableKey += 1;
            this.getSoftwCategories();
    ...
    getSoftwCategories() {
      axios.get(`http://localhost:8001/api/softwcategoriesjoin`).then((res) => {
        this.softwCategoryData = res.data;
        this.loading = false;
      });
    },
  ...
  setup() {
    const catColumns = [
      {
        name: "softw_category_type",
        label: "Software Category",
        field: "SOFTW_SUB_CATEGORY_DESC",
        align: "left",
      },
      {
        name: "status",
